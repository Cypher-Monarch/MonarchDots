...
if [[ "$1" == "rofi" ]]; then
  ...
  # Open rofi
  choice=$(echo -e "$options" | rofi -dmenu -replace -config ~/.config/rofi/hyprshade.rasi -i -no-show-icons -width 30 -p "Hyprshade")
  if [ ! -z $choice ]; then
    echo "hyprshade_filter=\"$choice\"" >~/.config/misc/settings/hyprshade.sh
    if [ "$choice" == "off" ]; then
      hyprshade off
  ...
  if [ -f ~/.config/misc/settings/hyprshade.sh ]; then
    source ~/.config/misc/settings/hyprshade.sh
  fi
